<template>
  <div class="flex-col">
    ...
    <template v-if="!isLoading">
      ...
      <table>
        <tr v-for="m in markests" :key="`${m.exchangeId}-${m.priceUsd}`" class="border-b">
          <td>
            <b> {{ m.exchangeId }} </b>
          </td>
  ...
  methods: {
    ...
    getWebSite (exchange) {
      ...
      return api.getExchange(exchange.exchangeId)
        .then(res => {
          ...
          // el metodo set sirve para poder mostrar los elementos que sean craqueados cuando la pagina ya esta cargada y poder mostrarlos
        })
        .finally(() => {
    ...
    getCoin() {
      ...
      Promise.all([api.getAsset(id), api.getAssetHistory(id), api.getMarkets(id)]).then(
        ([asset, history, markests]) => {
          this.asset = asset
          this.history = history
          this.markests = markests
        }
      )
